...
from utils import (
    ...
    normalize_payment_reference_key,
    parse_mpesa_u_code_from_transaction_details,
...
def _is_cash_payment_method(method) -> bool:
    return (method or "").strip().lower() == "cash"


def _normalized_ref_set_for_bank_transaction(tx: dict) -> set:
    """M-Pesa-style refs extracted from one bank line (U-code + narration)."""
    keys = set()
    u = str(tx.get("mpesa_u_code") or "").strip()
    if u:
        keys.add(normalize_payment_reference_key(u))
    desc = str(tx.get("description") or "")
    u2 = (parse_mpesa_u_code_from_transaction_details(desc) or "").strip()
    if u2:
        keys.add(normalize_payment_reference_key(u2))
    for m in re.finditer(r"\bU[A-Z0-9]{6,}\b", desc.upper()):
        keys.add(normalize_payment_reference_key(m.group(0)))
    return {k for k in keys if k}


def _student_has_payment_with_any_ref(conn, student_id, ref_keys: set) -> bool:
    if not ref_keys:
        return False
    sid = int(student_id)
    for (tid, iid) in conn.execute(
        "SELECT transaction_id, internal_payment_id FROM payments WHERE student_id=?",
        (sid,),
    ).fetchall():
        for part in (tid, iid):
            k = normalize_payment_reference_key(str(part or ""))
            if k and k in ref_keys:
                return True
    return False


def _auto_approve_pending_bank_drafts_from_statement(conn, transactions, df) -> tuple[int, list]:
    """
    If a pending **non-cash** manual payment's transaction ref appears on the statement with the same
    amount and matched student (score ≥ 40), record it without the pending-review admin password.
    """
    drafts = list(st.session_state.get("pending_manual_payment_drafts") or [])
    if not drafts:
        return 0, []
    n = 0
    errs = []
    used = set()
    max_pos = min(len(transactions), len(df))
    for pos in range(max_pos):
        ...
        if int(row.get("score") or 0) < 40:
            continue
        rk = _normalized_ref_set_for_bank_transaction(tx)
        if not rk:
            continue
        ...
        for d in drafts:
            ...
            if not did or did in used:
                continue
            if _is_cash_payment_method(d.get("payment_method")):
                continue
            dk = normalize_payment_reference_key(str(d.get("transaction_id") or ""))
            if not dk or dk not in rk:
                continue
            if int(d.get("student_id") or 0) != sid:
                continue
            if abs(float(d.get("amount") or 0) - amt) > 0.02:
                continue
            try:
                res_pay = _insert_manual_payment_row(
                    ...
                    payment_date_iso=str(d["payment_date"]),
                    payment_method=str(d.get("payment_method") or ""),
                    purpose=str(d.get("purpose") or ""),
                    transaction_id=str(d.get("transaction_id") or ""),
                    description_notes=str(d.get("description") or ""),
                )
                apply_optional_other_payer_for_payment(
                    conn,
                    int(d["student_id"]),
                    res_pay["payment_id"],
                    d.get("other_payer_name"),
                    d.get("other_payer_phone"),
                )
                _finalize_student_balance_after_payment(conn, int(d["student_id"]))
                remove_pending_manual_payment(conn, did)
                qb = d.get("queued_by_gate_user") or "unknown"
                _audit_log(
                    conn,
                    "Payment",
                    f"Auto-approved pending payment {res_pay['internal_payment_id']} (KSH {float(d['amount']):,.0f}) "
                    f"for student {d['student_id']} — bank statement matched transaction ref (no pending-review password). "
                    f"Draft had been queued by {qb}.",
                    save_mode="approved_from_bank_statement",
                    internal_payment_id=res_pay["internal_payment_id"],
                    detail=json.dumps(
                        {"draft_id": str(did), "queued_by": qb, "statement_row": pos},
                        default=str,
                    ),
                    entity_type="student",
                    entity_id=int(d["student_id"]),
                )
                used.add(did)
                n += 1
            except Exception as ex:
                errs.append(f"Draft {did}: {ex}")
    return n, errs


def _bank_commit_high_confidence_statement_rows(conn, df, transactions, *, min_score: int = 70) -> tuple[int, set, list]:
    """Insert payments for matched statement lines (score ≥ min_score), skipping duplicates by ref."""
    processed = 0
    affected_ids = set()
    bank_refs = []
    n = len(df)
    for pos in range(min(n, len(transactions))):
        row = df.iloc[pos]
        if row.get("student_id") is None:
            continue
        ...
        tx = transactions[pos]
        sid = int(row["student_id"])
        ref_keys = _normalized_ref_set_for_bank_transaction(tx)
        if _student_has_payment_with_any_ref(conn, sid, ref_keys):
            continue
        _base_ref = (
            (str(row.get("mpesa_u_code") or "").strip())
            or (str(row.get("description") or ""))
        )[:200]
        _ref = _ensure_unique_payment_reference(conn, _base_ref or new_payment_alnum_ref(9))
        bank_refs.append(_ref)
        _pdate = str(tx.get("date") or "").strip()[:10]
        if not _pdate or _pdate.lower() == "nan":
            _pdate = datetime.now().strftime("%Y-%m-%d")
        _pay_ts = f"{_pdate} 12:00:00"
        conn.execute(
            """
        ...
        conn.execute(
            ...
            WHERE id = ?
            """,
            (float(row["amount"]), sid),
        )
        affected_ids.add(sid)
        processed += 1
    conn.commit()
    for _sid in affected_ids:
        _finalize_student_balance_after_payment(conn, _sid)
    invalidate_student_cache()
    return processed, affected_ids, bank_refs


def _maybe_auto_process_bank_statement_upload(conn, file, transactions, df, students_df) -> Optional[dict]:
    """
    On every matching-results view: auto-approve pending **non-cash** drafts when ref+amount+student align
    with a statement line (score ≥ 40).

    High-confidence (≥70) statement lines are written to payment history **once per file** (content hash),
    skipping rows whose ref already exists for that student.
    """
    raw = file.getvalue()
    name = getattr(file, "name", None) or ""
    sig = hashlib.sha256(
        name.encode("utf-8", errors="replace") + str(len(raw)).encode("ascii") + raw[:20000]
    ).hexdigest()
    ap, ae = _auto_approve_pending_bank_drafts_from_statement(conn, transactions, df)

    stmt_n = 0
    refs: list = []
    if st.session_state.get("_bank_stmt_auto_sig") != sig:
        stmt_n, _aff, refs = _bank_commit_high_confidence_statement_rows(conn, df, transactions, min_score=70)
        st.session_state["_bank_stmt_auto_sig"] = sig
    msg_bits = []
    if ap:
        msg_bits.append(f"Auto-approved **{ap}** pending bank payment(s) using statement refs.")
    if ae:
        msg_bits.append("Notes: " + "; ".join(ae[:5]))
    if stmt_n:
        msg_bits.append(f"Auto-recorded **{stmt_n}** high-confidence bank line(s) (score ≥ 70).")
    if ap or stmt_n:
        _audit_log(
            conn,
            "Payment",
            "Bank statement upload: automatic import from file.",
            save_mode="immediate",
            detail=json.dumps(
                {
                    "pending_auto_approved": ap,
                    "statement_auto_rows": stmt_n,
                    "refs": refs[:200],
                    "pending_errors": ae[:20],
                },
                default=str,
            )[:8000],
        )
        st.session_state["_payment_flash_msg"] = " ".join(msg_bits)
        return {"rerun": True, "pending_approved": ap, "statement_rows": stmt_n}
    return {"rerun": False, "pending_approved": 0, "statement_rows": 0}
...
def _payment_history_add_outstanding_after(df):
    """
    Per approved payment row: outstanding **after** that payment was recorded, derived from the
    learner's **current** stored balance plus the sum of **newer** approved payments (same student,
    newer by date then id). Pending drafts are not in ``payments`` and do not affect this.
    """
    if df is None or df.empty:
        return df
    out = df.copy()
    out["_pd"] = pd.to_datetime(out["payment_date"], errors="coerce")
    raw = pd.Series(index=out.index, dtype="float64")
    for _, g in out.groupby("student_id", sort=False):
        g2 = g.sort_values(["_pd", "id"], ascending=[False, False])
        base = float(g2["student_balance"].iloc[0] or 0)
        s = g2["amount"].astype(float)
        prev_newer = s.cumsum() - s
        raw.loc[g2.index] = (base + prev_newer).astype(float).to_numpy()
    out["Outstanding after payment (KSH)"] = raw
    return out.drop(columns=["_pd"])
...
def _ensure_unique_payment_reference(conn, base: str, *, max_len: int = 200) -> str:
    """Return a reference string not yet used on payments (internal_payment_id / transaction_id)."""
    b = (base or "").strip()
    if not b:
        b = new_payment_alnum_ref(9)
    b = b[:max_len]
    cand = b
    seq = 0
    while True:
        hit = conn.execute(
            "SELECT 1 FROM payments WHERE internal_payment_id = ? OR transaction_id = ? LIMIT 1",
            (cand, cand),
        ).fetchone()
        if not hit:
            return cand
        seq += 1
        suf = f"#{seq}"
        cand = (b[: max_len - len(suf)] + suf)[:max_len]


def _manual_payment_reference_for_insert(conn, transaction_id: str, payment_method: str) -> str:
    """Canonical payment reference for manual Add payment (same value stored on transaction_id and internal_payment_id)."""
    pm = (payment_method or "").strip().lower()
    tx = (transaction_id or "").strip()
    if not tx and pm == "cash":
        return _ensure_unique_payment_reference(conn, new_payment_alnum_ref(9))
    if not tx:
        raise ValueError("Transaction / reference required for this payment method.")
    tid = tx[:200]
    hit = conn.execute(
        "SELECT 1 FROM payments WHERE internal_payment_id = ? OR transaction_id = ? LIMIT 1",
        (tid, tid),
    ...
    if hit:
        raise ValueError(
            f"This payment reference is already recorded: {tid!r}. "
            "Use a different code or delete the duplicate payment row first."
        )
    return tid


def _finalize_student_balance_after_payment(conn, student_id):
    """Recalculate stored balance after total_paid changes (manual or bank import)."""
    sid = int(student_id)
    sync_student_fees_from_db(conn, sid, do_commit=True)
    if get_current_term(conn):
        recompute_student_balance_from_ledger(conn, sid, do_commit=True)
...
def _insert_manual_payment_row(
    ...
    *,
    ...
    ref = _manual_payment_reference_for_insert(conn, transaction_id or "", pm)
    cur = conn.execute(
        """
        INSERT INTO payments (student_id, amount, payment_date, transaction_id, description, matched, purpose, payment_method, internal_payment_id)
        VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?)
    ...
    conn.commit()
    _finalize_student_balance_after_payment(conn, int(student_id))
    invalidate_student_cache()
    return {"internal_payment_id": ref, "payment_id": int(cur.lastrowid)}
...
def format_club_draft_detail_html(conn, draft):
    """Full pending-review HTML for club drafts (assign or import)."""
    kind = draft.get("kind")
    p = draft.get("payload") or {}
    ...
    names_li = f'<li><strong>Students (selection):</strong> {names_esc}</li>' if names_esc else ""
    head = f"<p><strong>{label}</strong></p><ul>{names_li}"
    if kind == "club_assign":
        ...
        if cid is not None:
            r = conn.execute(
                "SELECT fee_name FROM fee_structure WHERE id=? AND fee_category='co_curricular'",
                (int(cid),),
            ).fetchone()
            if r:
                cname = str(r[0] or "").strip() or f"id {cid}"
        head += (
            f"<li><strong>Mode:</strong> {html_module.escape(mode)}</li>"
            f"<li><strong>Club:</strong> {html_module.escape(cname)}</li>"
            f"<li><strong>Learners in roster:</strong> {n}</li></ul>"
        )
        return head
    mode = html_module.escape(str(p.get("mode") or "add"))
    rows = p.get("rows") or []
    head += (
        f"<li><strong>Import mode:</strong> {mode}</li>"
        f"<li><strong>Rows in file:</strong> {len(rows)}</li></ul>"
    )
    if not rows:
        return head
    cap = 120
    lines = []
    for i, rr in enumerate(rows[:cap]):
        if not isinstance(rr, dict):
            continue
        cn = html_module.escape(str(rr.get("club_name") or "—"))
        sn = html_module.escape(str(rr.get("student_name") or "—"))
        sr = rr.get("sheet_row", "")
        lines.append(f"<tr><td>{cn}</td><td>{sn}</td><td>{html_module.escape(str(sr))}</td></tr>")
    tail = (
        "<table style='width:100%; border-collapse:collapse; font-size:0.88rem; margin-top:0.5rem;'>"
        "<thead><tr><th align='left'>Club</th><th align='left'>Student name (as typed)</th>"
        "<th align='left'>Sheet row</th></tr></thead><tbody>"
        + "".join(lines)
        + "</tbody></table>"
    )
    if len(rows) > cap:
        tail += f"<p><em>Showing first {cap} of {len(rows)} import row(s).</em></p>"
    return head + tail


def format_grade_contact_draft_detail_html(conn, draft):
    p = draft.get("payload") or {}
    label = html_module.escape(draft.get("label") or "Grade contact changes")
    head = f"<p><strong>{label}</strong></p>"
    if draft.get("kind") == "grade_bulk":
        grade = str(p.get("grade") or "")
        base = _grade_contact_bulk_dataframe(conn, grade)
        rows_in = p.get("rows") or []
        if base.empty:
            return head + "<p><em>No active students in this grade.</em></p>"
        edited = _contact_rows_to_dataframe(rows_in, base)
        body = []
        for _, r in edited.iterrows():
            sid = int(r["id"])
            ob = base.loc[base["id"] == sid]
            if ob.empty:
                continue
            o0 = ob.iloc[0]
            bits = []
            for col, disp in (
                ...
                a = str(o0.get(col) or "").strip()
                b = str(r.get(col) or "").strip()
                if a != b:
                    bits.append(
                        f"<li><strong>{html_module.escape(disp)}:</strong> "
                        f"{html_module.escape(a or '—')} → {html_module.escape(b or '—')}</li>"
                    )
            if bits:
                body.append(
                    f"<div style='margin:0.4rem 0;'><strong>{html_module.escape(str(r.get('name') or sid))}</strong>"
                    f" <span style='color:#64748b;'>({_student_code_display_cell(r.get('student_code'))})</span>"
                    f"<ul style='margin:0.25rem 0 0.5rem 1rem;'>{''.join(bits)}</ul></div>"
                )
        if not body:
            return head + "<p>No contact field changes vs the current grade snapshot.</p>"
        return head + "".join(body[:200]) + (
            f"<p><em>…and {len(body) - 200} more learner(s).</em></p>" if len(body) > 200 else ""
        )
    rows = p.get("rows") or []
    head += f"<p><strong>Contact import rows:</strong> {len(rows)}</p>"
    if not rows:
        return head
    cap = 100
    lines = []
    for rr in rows[:cap]:
        if not isinstance(rr, dict):
            continue
        g = html_module.escape(str(rr.get("grade") or "—"))
        nm = html_module.escape(str(rr.get("student_name") or rr.get("name") or "—"))
        p1 = html_module.escape(str(rr.get("parent_name") or "—"))
        ph = html_module.escape(str(rr.get("parent_phone") or "—"))
        p2n = html_module.escape(str(rr.get("parent2_name") or "—"))
        p2p = html_module.escape(str(rr.get("parent2_phone") or "—"))
        dob = html_module.escape(str(rr.get("date_of_birth") or "—"))
        sr = html_module.escape(str(rr.get("sheet_row", "—")))
        lines.append(
            f"<tr><td>{g}</td><td>{nm}</td><td>{p1}</td><td>{ph}</td><td>{p2n}</td><td>{p2p}</td><td>{dob}</td><td>{sr}</td></tr>"
        )
    tbl = (
        "<table style='width:100%; border-collapse:collapse; font-size:0.85rem; margin-top:0.35rem;'>"
        "<thead><tr><th align='left'>Grade</th><th align='left'>Student</th><th align='left'>Parent 1</th>"
        "<th align='left'>Phone 1</th><th align='left'>Parent 2</th><th align='left'>Phone 2</th>"
        "<th align='left'>DOB</th><th align='left'>Sheet row</th></tr></thead><tbody>"
        + "".join(lines)
        + "</tbody></table>"
    )
    if len(rows) > cap:
        tbl += f"<p><em>Showing first {cap} of {len(rows)} row(s).</em></p>"
    return head + tbl


def format_balance_draft_detail_html(conn, draft):
    p = draft.get("payload") or {}
    ...
    if kind == "balance_bulk":
        ...
        if base.empty:
            return head + "<p><em>No active students in this grade.</em></p>"
        edited = _balance_rows_to_dataframe(rows_in, base)
        lines = []
        for _, r in edited.iterrows():
            sid = int(r["id"])
            ob = base.loc[base["id"] == sid]
            if ob.empty:
                continue
            old_b = str(ob.iloc[0].get("balance") or "")
            new_b = str(r.get("balance") or "")
            if old_b == new_b:
                continue
            nm = html_module.escape(str(r.get("name") or f"id {sid}"))
            code = html_module.escape(_student_code_display_cell(r.get("student_code")))
            lines.append(
                f"<tr><td>{nm}</td><td>{code}</td><td>{html_module.escape(old_b)}</td>"
                f"<td>{html_module.escape(new_b)}</td></tr>"
            )
        if not lines:
            return head + "<p>No balance text changes in this draft (matches current snapshot).</p>"
        cap = 400
        shown = lines[:cap]
        tbl = (
            "<p>Compared to the grade table snapshot when this was saved (confirm applies to <strong>current</strong> records).</p>"
            "<table style='width:100%; border-collapse:collapse; font-size:0.88rem;'>"
            "<thead><tr><th align='left'>Student</th><th align='left'>Code</th>"
            "<th align='left'>Balance (before)</th><th align='left'>Balance (draft)</th></tr></thead><tbody>"
            + "".join(shown)
            + "</tbody></table>"
        )
        if len(lines) > cap:
            tbl += f"<p><em>Showing first {cap} of {len(lines)} change(s).</em></p>"
        return head + tbl
    rows = p.get("rows") or []
    head += f"<p><strong>Balance import rows:</strong> {len(rows)}</p>"
    if not rows:
        return head
    cap = 150
    lines = []
    for rr in rows[:cap]:
        if not isinstance(rr, dict):
            continue
        g = html_module.escape(str(rr.get("grade") or "—"))
        nm = html_module.escape(str(rr.get("student_name") or "—"))
        st = html_module.escape(str(rr.get("balance_status") or "—"))
        bal = rr.get("balance")
        bal_s = "—" if bal is None else f"{float(bal):,.0f}"
        sr = html_module.escape(str(rr.get("sheet_row", "—")))
        lines.append(f"<tr><td>{g}</td><td>{nm}</td><td>{st}</td><td>{html_module.escape(bal_s)}</td><td>{sr}</td></tr>")
    tbl = (
        "<table style='width:100%; border-collapse:collapse; font-size:0.88rem; margin-top:0.35rem;'>"
        "<thead><tr><th align='left'>Grade</th><th align='left'>Name</th><th align='left'>Status</th>"
        "<th align='left'>Amount</th><th align='left'>Sheet row</th></tr></thead><tbody>"
        + "".join(lines)
        + "</tbody></table>"
    )
    if len(rows) > cap:
...
def _transport_route_label_map(conn):
    return {
        int(r[0]): str(r[1] or f"Route {r[0]}")
...
def format_meal_draft_detail_html(conn, draft):
    p = draft.get("payload") or {}
    ...
    rows_in = p.get("rows") or []
    if base.empty:
        return f"<p><strong>{label}</strong></p><p><em>No active students in this grade.</em></p>"
    edited = _meal_rows_to_dataframe(rows_in, base)
    lines = []
    for _, r in edited.iterrows():
        sid = int(r["id"])
        ob = base.loc[base["id"] == sid]
        if ob.empty:
            continue
        old_m = "Yes" if int(ob.iloc[0].get("has_meal") or 0) else "No"
        new_m = str(r.get("meals") or "").strip()
        if old_m == new_m:
            continue
        nm = html_module.escape(str(r.get("name") or f"id {sid}"))
        code = html_module.escape(_student_code_display_cell(r.get("student_code")))
        lines.append(
            f"<tr><td>{nm}</td><td>{code}</td><td>{html_module.escape(old_m)}</td>"
            f"<td>{html_module.escape(new_m)}</td></tr>"
        )
    head = f"<p><strong>{label}</strong></p><p><strong>Grade:</strong> {html_module.escape(grade)}</p>"
    if not lines:
        return head + "<p>No meal Yes/No changes in this draft.</p>"
    cap = 400
    shown = lines[:cap]
    tbl = (
        "<table style='width:100%; border-collapse:collapse; font-size:0.88rem;'>"
        "<thead><tr><th align='left'>Student</th><th align='left'>Code</th>"
        "<th align='left'>Meals (before)</th><th align='left'>Meals (draft)</th></tr></thead><tbody>"
        + "".join(shown)
        + "</tbody></table>"
    )
    if len(lines) > cap:
...
def format_transport_draft_detail_html(conn, draft):
    p = draft.get("payload") or {}
    label = html_module.escape(draft.get("label") or "Transport changes")
    grade = str(p.get("grade") or "")
    base = _transport_users_bulk_dataframe(conn, grade)
    rmap = _transport_route_label_map(conn)
    rows_in = p.get("rows") or []
    if base.empty:
        return f"<p><strong>{label}</strong></p><p><em>No active students in this grade.</em></p>"
    edited = _transport_rows_to_dataframe(rows_in, base)

    def _route_disp(tc):
        tc = str(tc or "__none__").strip()
        if tc in ("", "__none__", "none"):
            return "No transport"
        try:
            return rmap.get(int(tc), tc)
        except (TypeError, ValueError):
            return tc
    ...
    for _, r in edited.iterrows():
        ...
        if ob.empty:
            continue
        old_tc = str(ob.iloc[0].get("transport_choice") or "__none__").strip()
        new_tc = str(r.get("transport_choice") or "__none__").strip()
        if old_tc == new_tc:
            continue
        nm = html_module.escape(str(r.get("name") or f"id {sid}"))
        code = html_module.escape(_student_code_display_cell(r.get("student_code")))
        o_lab = html_module.escape(str(_route_disp(old_tc)))
        n_lab = html_module.escape(str(_route_disp(new_tc)))
        lines.append(f"<tr><td>{nm}</td><td>{code}</td><td>{o_lab}</td><td>{n_lab}</td></tr>")
    head = f"<p><strong>{label}</strong></p><p><strong>Grade:</strong> {html_module.escape(grade)}</p>"
    if not lines:
        return head + "<p>No transport route changes in this draft.</p>"
    cap = 400
    shown = lines[:cap]
    tbl = (
        "<table style='width:100%; border-collapse:collapse; font-size:0.88rem;'>"
        "<thead><tr><th align='left'>Student</th><th align='left'>Code</th>"
        "<th align='left'>Transport (before)</th><th align='left'>Transport (draft)</th></tr></thead><tbody>"
        + "".join(shown)
        + "</tbody></table>"
    )
    if len(lines) > cap:
...
def format_pending_manual_payment_detail_html(conn, draft):
    """Full-field HTML for a queued manual payment draft."""
    sid = draft.get("student_id")
    slab = "—"
    if sid is not None:
        r = conn.execute(
            "SELECT name, student_code FROM students WHERE id=?",
            (int(sid),),
        ).fetchone()
        if r:
            slab = f"{r[0]} ({_student_code_display_cell(r[1])})"
    qb = html_module.escape(str(draft.get("queued_by_gate_user") or "—"))
    did = html_module.escape(str(draft.get("id") or "—"))
    tx = html_module.escape(str(draft.get("transaction_id") or "").strip() or "—")
    opn = html_module.escape(str(draft.get("other_payer_name") or "").strip() or "—")
    opp = html_module.escape(str(draft.get("other_payer_phone") or "").strip() or "—")
    ...
    return (
        ...
        f"<li><strong>Student:</strong> {html_module.escape(slab)}</li>"
        f"<li><strong>Amount (KSH):</strong> {html_module.escape(str(draft.get('amount', '—')))}</li>"
        f"<li><strong>Payment date:</strong> {html_module.escape(str(draft.get('payment_date', '—')))}</li>"
        f"<li><strong>Method:</strong> {html_module.escape(str(draft.get('payment_method', '—')))}</li>"
        f"<li><strong>Purpose:</strong> {html_module.escape(str(draft.get('purpose', '—')))}</li>"
        f"<li><strong>Transaction / reference:</strong> {tx}</li>"
        f"<li><strong>Other payer name:</strong> {opn}</li>"
        f"<li><strong>Other payer phone:</strong> {opp}</li>"
        f"<li><strong>Notes / description:</strong> {desc}</li>"
        "</ul>"
    )


def format_pending_expense_detail_html(draft):
    ...
    return (
        ...
        f"<li><strong>Amount (KSH):</strong> {html_module.escape(str(draft.get('amount', '—')))}</li>"
        f"<li><strong>Method:</strong> {html_module.escape(str(draft.get('payment_method', '—')))}</li>"
        f"<li><strong>Vendor:</strong> {html_module.escape(str(draft.get('vendor', '—')))}</li>"
        f"<li><strong>Receipt #:</strong> {html_module.escape(str(draft.get('receipt_number', '—')))}</li>"
        f"<li><strong>Description:</strong> {desc}</li>"
        "</ul>"
    )
...
def _apply_all_pending_manual_payments(conn):
    ...
    for _d in list(st.session_state.get("pending_manual_payment_drafts") or []):
        ...
        try:
            ...
            _finalize_student_balance_after_payment(conn, int(_d["student_id"]))
...
def _render_pending_reviews_tab(conn):
    ...
    if _action_sel in ("All actions", "Clubs"):
        for draft in _club_drafts:
            ...
            render_pending_apply_workflow(
                _dk,
                format_club_draft_detail_html(conn, draft),
                _apply_club,
                _discard_club,
    ...
    if _action_sel in ("All actions", "Grade & contact"):
        for draft in _grade_drafts:
            ...
            render_pending_apply_workflow(
                _dk,
                format_grade_contact_draft_detail_html(conn, draft),
                _apply_grade,
                _discard_grade,
    ...
    if _action_sel in ("All actions", "Balance"):
        for draft in _bal_drafts:
            ...
            render_pending_apply_workflow(
                _dk,
                format_balance_draft_detail_html(conn, draft),
                _apply_bal,
                _discard_bal,
    ...
    if _action_sel in ("All actions", "Meals"):
        for draft in _meal_drafts:
            ...
            render_pending_apply_workflow(
                _dk,
                format_meal_draft_detail_html(conn, draft),
                _apply_meal,
                _discard_meal,
    ...
    if _action_sel in ("All actions", "Transport"):
        for draft in _transport_drafts:
            ...
            render_pending_apply_workflow(
                _dk,
                format_transport_draft_detail_html(conn, draft),
                _apply_tr,
                _discard_tr,
...
def _render_pending_manual_payments_tab(conn):
    ...
    for _d in _pend:
        ...
        _summary = format_pending_manual_payment_detail_html(conn, _d)
...
def _render_pending_expenses_tab(conn):
    ...
    for _d in _pend:
        ...
        _summary = format_pending_expense_detail_html(_d)
...
def _render_bank_statement_upload(conn):
    ...
    file = st.file_uploader(
        ...
        "After parsing, high-confidence matches (score ≥ 70) are written to payment history automatically; pending **non-cash** "
        "payments whose transaction code appears on the statement can be approved without the pending-review admin password. "
        "**Cash** pending payments still require manual approval with the admin password.",
    )
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    try:
        _auto_stmt = _maybe_auto_process_bank_statement_upload(conn, file, transactions, df, students)
        if _auto_stmt and _auto_stmt.get("rerun"):
            st.rerun()
    except Exception as ex:
        st.warning(
            f"Automatic statement processing could not complete: {html_module.escape(str(ex))}"
        )
    ...
    st.caption(
        "Rows with **High** confidence are already recorded when possible (once per upload). "
        "Use **Process Selected Payments** below to import additional rows by confidence with an **admin password**. "
        "**Cash** payments queued for review always need approval with the admin password."
    )
                ...
                try:
                    ...
                    for _, row in df_filtered.iterrows():
                        if row["student_id"] is not None:
                            ...
                            _ref = _ensure_unique_payment_reference(
                                conn, _base_ref or new_payment_alnum_ref(9)
                            )
                            bank_ipids.append(_ref)
                            _pdate = str(row.get("date") or "").strip()[:10]
                            if not _pdate or _pdate.lower() == "nan":
                                _pdate = datetime.now().strftime("%Y-%m-%d")
                            _pay_ts = f"{_pdate} 12:00:00"
                            conn.execute(
...
def _render_manual_payment_tab(conn):
    ...
    st.caption(
        "**Save now** (admin password) writes the row to **Payment History** and recalculates balances immediately. "
        "**Save for later** keeps a draft under **Pending Reviews** only — it does **not** appear in Payment History "
        "or receipts until approved; balances change only after approval."
    )
    ...
    with st.form("manual_payment_form"):
        ...
        if save_later:
            ...
            if not ok:
                ...
                st.session_state["_payment_flash_msg"] = (
                    "Payment saved for later — it is **not** in **Payment History** or receipts until you approve it "
                    "under **Payment Management → Pending Reviews** (balances update only after approval)."
                )
                st.rerun()
...
if menu == "Dashboard":
    ...
    st.caption(
        ...
        "After a statement is parsed, **high-confidence** matches are written to payment history automatically (once per file); "
        "pending **non-cash** payments whose transaction code appears on the statement can be approved the same way—**cash** "
        "pending items still need the pending-review admin password. "
    ...
    with tab_pay:
        if not template_path_resolved:
            ...
            _gr_pay_dates = pd.to_datetime(pay_merged["payment_date"], errors="coerce")
            _gr_tf_col, _gr_search_col = st.columns([1, 2])
            with _gr_tf_col:
                gr_time_filter = st.selectbox(
                    "Time range",
                    ("All", "Last 24 hours", "Last 7 days", "Last 30 days", "Last 90 days"),
                    key="gr_pay_time_filter",
                    help="Only payments on or after this cutoff are listed; search applies within this set.",
                )
            with _gr_search_col:
                pay_search = st.text_input(
                    "Search payments",
                    placeholder="Student name, code, parent phone, bank ref, internal ID, amount…",
                    help="Filters the list below. Empty search shows the 250 most recent payments in the selected time range.",
                    key="gr_pay_search_main",
                )
            if gr_time_filter == "All":
                pay_scoped = pay_merged
            elif gr_time_filter == "Last 24 hours":
                _gr_cut = pd.Timestamp.now() - pd.Timedelta(hours=24)
                pay_scoped = pay_merged[_gr_pay_dates >= _gr_cut].copy()
            else:
                _gr_days = {"Last 7 days": 7, "Last 30 days": 30, "Last 90 days": 90}[gr_time_filter]
                _gr_cut = pd.Timestamp.now() - pd.Timedelta(days=_gr_days)
                pay_scoped = pay_merged[_gr_pay_dates >= _gr_cut].copy()

            _needle = (pay_search or "").strip().lower()
            if not _needle:
                filtered = pay_scoped.head(250)
            else:
                ...
                filtered = pay_scoped[pay_scoped.apply(_pay_row_hit, axis=1)]
            ...
            elif pay_scoped.empty:
                st.info("No payments fall in the selected time range. Choose **All** or a wider window.")
            elif filtered.empty:
                    ...
                    st.session_state.gr_editor_df = pd.DataFrame(
                        {
                            ...
                            "Reference": filtered.apply(payment_display_reference, axis=1),
                        }
                    )
    ...
    st.info(
        "**Approved payments only.** Rows here come from the live `payments` table after **Save now** "
        "or **Pending Reviews → approve** (admin password). **Save for later** drafts are **not** listed "
        "here and do **not** change balances or receipts until they are approved. After approval, balances "
        "are recalculated from fees and totals, then the payment appears here for **Generate receipts**."
    )
    ...
    with search_col1:
        search_term = st.text_input(
            "Search by Name/Code/Phone/Reference",
            placeholder="Student name, code, phone, payment reference…",
            help="Matches approved payment rows only (pending drafts are not in this list). "
            "Searches name/code/parent phones, payment reference, description, method, or purpose.",
    ...
    query = """
        SELECT p.*, s.name as student_name, s.student_code, s.grade, s.parent_name,
               ...
               s.balance_status as student_balance_status,
               s.balance_set as student_balance_set,
               s.total_paid as student_total_paid,
               s.is_sponsored as student_is_sponsored
        FROM payments p
        JOIN students s ON p.student_id = s.id
        WHERE 1=1
    """
    ...
    if not payments_df.empty:
        payments_df = _payment_history_add_outstanding_after(payments_df)
        ...
        _ref_col = payments_df.apply(payment_display_reference, axis=1).replace("", "—")
        _bst = payments_df["student_balance_status"].fillna("").astype(str).str.strip()
        _mask_not_set = _bst == BALANCE_STATUS_NOT_SET
        _sp = (
            payments_df["student_is_sponsored"].fillna(0).astype(int) == 1
            if "student_is_sponsored" in payments_df.columns
            else pd.Series(False, index=payments_df.index)
        )
        _v_after = payments_df["Outstanding after payment (KSH)"]
        _out_disp = []
        for m, sp, v in zip(_mask_not_set, _sp, _v_after):
            if m or sp or v is None or (isinstance(v, float) and pd.isna(v)):
                _out_disp.append("—")
            else:
                _out_disp.append(f"KSH {float(v):,.0f}")
        ...
        display_df = pd.DataFrame(
            {
                ...
                "Payment reference": _ref_col,
                "Outstanding after payment": _out_disp,
                "Other details": _desc,
        ...
        st.caption(
            "**Outstanding after payment** is reconstructed from the learner's **current** stored balance plus "
            "all **newer** approved payments (same student), so it reflects *balance after this row* when "
            "billing rules have been stable. Shows **—** when the learner balance is **Not set** or the learner is **sponsored**."
        )
        st.markdown('<div class="dataframe-container">', unsafe_allow_html=True)
        st.dataframe(
        ...
        st.markdown(
            '<div class="info-message">No approved payment rows match these filters. '
            "Drafts saved for later stay under **Pending Reviews** until approved — they never appear here.</div>",
            unsafe_allow_html=True,
        )
