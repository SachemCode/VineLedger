...
def generate_receipt(student, amount, receipt_number=None, payment=None):
    ...
    if payment is not None:
        _tid = _receipt_safe_str(_row_get(payment, "internal_payment_id", ""), "").strip()
        if _tid:
            ...
            story.append(Paragraph(html.escape(_tid[:56]), tid_style))
...
def _receipt_field_values(student, payment, receipt_number):
    ...
    txid = _receipt_safe_str(_row_get(payment, "transaction_id", ""), "N/A")
    ...
    internal_pay_id = _receipt_safe_str(_row_get(payment, "internal_payment_id", ""), "").strip()
...
def _draw_filled_receipt_panel(
    ...
    if _tid and not (fields and "internal_payment_id" in fields):
        ...
        _canvas_draw_text_with_halo(c, tshow, rx, ry, "Helvetica", fs_tid, "right", ink, halo)
